		...
		<form id="toolkit-application-form" class="application-form" novalidate>
			...
			<?php if ( ! toolkit_mzizi_submission_enabled() ) : ?>
				<div class="application-handoff-notice" role="note">
					<div><strong>Your application will be saved by Toolkit</strong><span>Direct Mzizi relay is not active in this environment. Complete the form to create a Toolkit reference, then continue to the official portal.</span></div>
					<a class="toolkit-btn toolkit-btn--primary" href="<?php echo esc_url( toolkit_mzizi_application_url() ); ?>" target="_blank" rel="noopener noreferrer">Open official portal now <i class="fas fa-external-link-alt" aria-hidden="true"></i></a>
				</div>
			<?php endif; ?>
			...
			<fieldset data-step-panel="5" hidden><legend class="screen-reader-text">Review and submit</legend><div id="application-review" class="application-review"></div><label class="application-consent"><input type="checkbox" name="consent" value="1" required><span>I confirm that the information is accurate and consent to The Toolkit for Skills and Innovation storing it securely and distributing it to Mzizi for admissions processing. I have read the <a href="<?php echo esc_url( home_url( '/privacy-policy/' ) ); ?>" target="_blank" rel="noopener">privacy notice</a>. <b>*</b></span></label><?php if ( toolkit_application_turnstile_site_key() ) : ?><div class="cf-turnstile" data-sitekey="<?php echo esc_attr( toolkit_application_turnstile_site_key() ); ?>"></div><?php endif; ?><p class="application-submit-note" data-submit-note></p></fieldset>
