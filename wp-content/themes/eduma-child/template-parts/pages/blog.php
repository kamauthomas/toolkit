...
<main id="main-content" class="toolkit-page toolkit-blog-page">
	...
	<section class="toolkit-blog-feature toolkit-section">
		<a href="<?php the_permalink(); ?>"><img src="<?php echo esc_url( toolkit_story_image_url( get_the_ID() ) ); ?>" width="780" height="520" alt="<?php echo esc_attr( get_the_title() ); ?>"></a>
	...
	<section class="toolkit-blog-feed toolkit-section" aria-labelledby="toolkit-blog-feed-title">
		<div class="toolkit-blog-feed__heading"><div><p class="toolkit-kicker">More from Toolkit</p><h2 id="toolkit-blog-feed-title">Recent stories and updates</h2></div><p>Programme news, conversations and practical insight from Toolkit teams and partners. New here? Start with <a href="<?php echo esc_url( home_url( '/about-toolkit-africa/' ) ); ?>">the Toolkit story and training model</a>.</p></div>
		<div class="toolkit-blog-grid"><?php $index = 1; while ( $posts->have_posts() ) : $posts->the_post(); ?><article><a class="toolkit-blog-card__image" href="<?php the_permalink(); ?>"><img src="<?php echo esc_url( toolkit_story_image_url( get_the_ID(), 'large', $index ) ); ?>" width="620" height="380" loading="lazy" alt="<?php echo esc_attr( get_the_title() ); ?>"></a><div><time datetime="<?php echo esc_attr( get_the_date( DATE_W3C ) ); ?>"><?php echo esc_html( get_the_date() ); ?></time><h3><a href="<?php the_permalink(); ?>"><?php the_title(); ?></a></h3><p><?php echo esc_html( wp_trim_words( get_the_excerpt(), 22 ) ); ?></p><a href="<?php the_permalink(); ?>">Read story <i class="fas fa-arrow-right" aria-hidden="true"></i></a></div></article><?php $index++; endwhile; ?></div>
	</section>
	<?php else : ?>
