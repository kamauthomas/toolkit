...
command="/usr/bin/flock -n /tmp/toolkit-campus-test-deploy.lock -c 'cd $private_root && /usr/local/bin/ea-php84 artisan storage:link > $log 2>&1 && /usr/local/bin/ea-php84 artisan optimize:clear >> $log 2>&1 && /usr/local/bin/ea-php84 artisan config:cache >> $log 2>&1 && /usr/local/bin/ea-php84 artisan campus:readiness --json >> $log 2>&1 && echo CAMPUS_READINESS_OK >> $log'"
...
for _ in {1..12}; do
  sleep 10
  if fetch "$private_root/storage/logs" "deploy-$release.log" "$rollback/deploy.log" 2>/dev/null && rg -q 'CAMPUS_READINESS_OK' "$rollback/deploy.log"; then break; fi
done
rg -q 'CAMPUS_READINESS_OK' "$rollback/deploy.log" || { sed -n '1,160p' "$rollback/deploy.log" >&2; exit 14; }
rg -q '"ready": true' "$rollback/deploy.log"
rg -q 'production captions remain blocked' "$rollback/deploy.log"
...
printf 'environment=staging release=%s commit=%s url=%s readiness=verified rollback=%s\n' "$release" "$commit" "$public_base" "$rollback"
