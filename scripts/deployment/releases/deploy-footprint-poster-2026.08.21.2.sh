...
base_ref=toolkit-release-2026.08.21.1
target_ref=${TOOLKIT_DEPLOY_TARGET_REF:-toolkit-release-2026.08.21.2}
...
rg -q 'toolkit-footprint-columns' "$work/wp-content/themes/eduma-child/page-redesign.css"
rg -q 'Toolkit footprint in youth skills and employability' "$work/wp-content/themes/eduma-child/template-parts/pages/footprint.php"
if rg -q 'toolkit-footprint-(focus|hero__inner)' "$work/wp-content/themes/eduma-child/page-redesign.css" "$work/wp-content/themes/eduma-child/template-parts/pages/footprint.php"; then
	printf 'superseded editorial footprint UI remains in payload\n' >&2
	exit 10
fi
...
rg -q 'Toolkit footprint in youth skills and employability' "$work/footprint.html"
rg -q 'toolkit-footprint-columns' "$work/footprint.html"
...
printf 'environment=%s release=%s rollback=%s route=200 poster=verified files=verified-byte-identical\n' \
